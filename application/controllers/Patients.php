...
class Patients extends MY_Controller {
    ...
    function index($tab = "") {
        ...
        $view_data["custom_field_headers"] = $this->Custom_fields_model->get_custom_field_headers_for_table("patients", $this->login_user->is_admin, $this->login_user->user_type);

        $view_data['groups_dropdown'] = json_encode($this->_get_patient_groups_dropdown_select2_data(true));
        $view_data['can_edit_patients'] = $this->can_edit_patients();
        $view_data["team_members_dropdown"] = $this->get_team_members_dropdown(true);

        $view_data['tab'] = $tab;

        $this->template->rander("patients/index", $view_data);
    }

    //for team members, check only read_only permission here, since other permission will be checked accordingly
    private function can_edit_patients() {
    ...
    private function can_view_files() {
        if ($this->login_user->user_type == "staff") {
            ...
            if (!get_setting("patient_can_view_files")) {
                redirect("forbidden");
            }
        }
    }
    ...
    private function can_access_this_patient($patient_id = 0) {
        $patient_info = $this->Patients_model->get_one($patient_id);

        if ($this->login_user->user_type === "staff" && $patient_info->id && get_array_value($this->login_user->permissions, "client") === "own" && !($patient_info->created_by == $this->login_user->id || $patient_info->owner_id == $this->login_user->id)) {
            redirect("forbidden");
        }
    }

    /* load patient add/edit modal */
    ...
    function modal_form() {
        ...
        $view_data["view"] = $this->input->post('view'); //view='details' needed only when loading from the patient's details view
        $view_data["ticket_id"] = $this->input->post('ticket_id'); //needed only when loading from the ticket's details view and created by unknown patient
        $view_data['model_info'] = $this->Patients_model->get_one($patient_id);
        ...
        //get custom fields
        $view_data["custom_fields"] = $this->Custom_fields_model->get_combined_details("patients", $patient_id, $this->login_user->is_admin, $this->login_user->user_type)->result();

        $this->load->view('patients/modal_form', $view_data);
    }
    ...
    function save() {
        ...
        $this->access_only_allowed_members_or_client_contact($patient_id);
        ...
        if ($this->login_user->is_admin || get_array_value($this->login_user->permissions, "patient") === "all") {
            ...
            //the user hasn't permission to change created by but s/he can create new patient
        ...
        if (get_setting("disallow_duplicate_patient_company_name") == "1" && $this->Patients_model->is_duplicate_company_name($data["company_name"], $patient_id)) {
            echo json_encode(array("success" => false, 'message' => lang("account_already_exists_for_your_company_name")));
            exit();
        }


        $save_id = $this->Patients_model->save($data, $patient_id);
        if ($save_id) {
            save_custom_fields("patients", $save_id, $this->login_user->is_admin, $this->login_user->user_type);

            //save patient id on the ticket if any ticket id exists
            $ticket_id = $this->input->post('ticket_id');
            if ($ticket_id) {
                $ticket_data = array("patient_id" => $save_id);
    ...
    function delete() {
        ...
        $this->can_access_this_patient($id);

        if ($this->Patients_model->delete_patient_and_sub_items($id)) {
    ...
    /* list of patients, prepared for datatable  */
    ...
    /* return a row of patient list  table */
    ...
    /* prepare a row of patient list table */
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $primary_contact = get_patient_contact_profile_link($data->primary_contact_id, $contact);
        ...
        $row_data = array($data->id,
            anchor(get_uri("patients/view/" . $data->id), $data->company_name),
        ...
        $row_data[] = modal_anchor(get_uri("patients/modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_patient'), "data-post-id" => $data->id))
                . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_patient'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("patients/delete"), "data-action" => "delete-confirmation"));
    ...
    function view($patient_id = 0, $tab = "") {
        $this->access_only_allowed_members();
        $this->can_access_this_patient($patient_id);

        if ($patient_id) {
            $options = array("id" => $patient_id);
            $patient_info = $this->Patients_model->get_details($options)->row();
            if ($patient_info && !$patient_info->is_lead) {
                ...
                $view_data["is_starred"] = strpos($patient_info->starred_by, ":" . $this->login_user->id . ":") ? true : false;

                $view_data["tab"] = $tab;

                $view_data["view_type"] = "";

                //even it's hidden, admin can view all information of patient
                $view_data['hidden_menu'] = array("");

                $this->template->rander("patients/view", $view_data);
            } else {
                show_404();
            }
        } else {
    ...
    /* add-remove start mark from patient */

    function add_remove_star($patient_id, $type = "add") {
        if ($patient_id) {
            $view_data["patient_id"] = $patient_id;

            if ($type === "add") {
                $this->Patients_model->add_remove_star($patient_id, $this->login_user->id, $type = "add");
                $this->load->view('patients/star/starred', $view_data);
            } else {
                $this->Patients_model->add_remove_star($patient_id, $this->login_user->id, $type = "remove");
                $this->load->view('patients/star/not_starred', $view_data);
            }
        }
    }

    function show_my_starred_patients() {
        $view_data["patients"] = $this->Patients_model->get_starred_patients($this->login_user->id)->result();
        $this->load->view('patients/star/patients_list', $view_data);
    }

    /* load projects tab  */

    function projects($patient_id) {
        $this->access_only_allowed_members();
        $this->can_access_this_patient($patient_id);
        ...
        $view_data['patient_id'] = $patient_id;
        $this->load->view("patients/projects/index", $view_data);
    }

    /* load payments tab  */

    function payments($patient_id) {
        $this->access_only_allowed_members();
        $this->can_access_this_patient($patient_id);
        ...
        if ($patient_id) {
            ...
            $view_data['patient_id'] = $patient_id;
            $this->load->view("patients/payments/index", $view_data);
        }
    }

    /* load tickets tab  */

    function tickets($patient_id) {
        $this->access_only_allowed_members();
        $this->can_access_this_patient($patient_id);
        ...
        if ($patient_id) {
            ...
            $this->load->view("patients/tickets/index", $view_data);
        }
    }

    /* load invoices tab  */

    function invoices($patient_id) {
        $this->access_only_allowed_members();
        $this->can_access_this_patient($patient_id);
        ...
        if ($patient_id) {
            ...
            $this->load->view("patients/invoices/index", $view_data);
        }
    }

    /* load estimates tab  */

    function estimates($patient_id) {
        $this->access_only_allowed_members();
        $this->can_access_this_patient($patient_id);
        ...
        if ($patient_id) {
            ...
            $this->load->view("patients/estimates/estimates", $view_data);
        }
    }

    /* load orders tab  */

    function orders($patient_id) {
        $this->access_only_allowed_members();
        ...
        if ($patient_id) {
            ...
            $this->load->view("patients/orders/orders", $view_data);
        }
    }

    /* load estimate requests tab  */

    function estimate_requests($patient_id) {
        $this->access_only_allowed_members();
        $this->can_access_this_patient($patient_id);

        if ($patient_id) {
            $view_data['patient_id'] = $patient_id;
            $this->load->view("patients/estimates/estimate_requests", $view_data);
        }
    }

    /* load notes tab  */

    function notes($patient_id) {
        $this->access_only_allowed_members();
        $this->can_access_this_patient($patient_id);

        if ($patient_id) {
            $view_data['patient_id'] = $patient_id;
            $this->load->view("patients/notes/index", $view_data);
        }
    }

    /* load events tab  */

    function events($patient_id) {
        $this->access_only_allowed_members();
        $this->can_access_this_patient($patient_id);

        if ($patient_id) {
            $view_data['patient_id'] = $patient_id;
            $view_data['calendar_filter_dropdown'] = $this->get_calendar_filter_dropdown("patient");
    ...
    function files($patient_id, $view_type = "") {
        $this->can_view_files();
        $this->can_access_this_patient($patient_id);

        if ($this->login_user->user_type == "patient") {
            $patient_id = $this->login_user->patient_id;
        }
        ...
        if ($view_type == "page_view") {
            ...
            $this->template->rander("patients/files/index", $view_data);
        } else {
            $this->load->view("patients/files/index", $view_data);
        }
    }
    ...
    function file_modal_form() {
        ...
        $patient_id = $this->input->post('patient_id') ? $this->input->post('patient_id') : $view_data['model_info']->patient_id;
        $this->can_access_this_patient($patient_id);

        $view_data['patient_id'] = $patient_id;
        $this->load->view('patients/files/modal_form', $view_data);
    }
    ...
    function save_file() {
        ...
        $target_path = getcwd() . "/" . get_general_file_path("patient", $patient_id);
    ...
    function files_list_data($patient_id = 0) {
        $this->can_view_files();
        $this->can_access_this_patient($patient_id);

        $options = array("patient_id" => $patient_id);
    ...
    private function _make_file_row($data) {
        ...
        if ($data->uploaded_by_user_type == "staff") {
            ...
            $uploaded_by = get_patient_contact_profile_link($data->uploaded_by, $uploaded_by);
        }

        $description = "<div class='pull-left'>" .
                js_anchor(remove_file_prefix($data->file_name), array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "data-url" => get_uri("patients/view_file/" . $data->id)));
        ...
        $options = anchor(get_uri("patients/download_file/" . $data->id), "<i class='fa fa fa-cloud-download'></i>", array("title" => lang("download")));

        if ($this->login_user->user_type == "staff") {
            $options .= js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_file'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("patients/delete_file"), "data-action" => "delete-confirmation"));
        }
    ...
    function view_file($file_id = 0) {
        ...
        if ($file_info) {
            ...
            $this->can_access_this_patient($file_info->patient_id);

            $view_data['can_comment_on_files'] = false;
            $file_url = get_source_url_of_file(make_array_of_file($file_info), get_general_file_path("patient", $file_info->patient_id));
            ...
            $this->load->view("patients/files/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function download_file($id) {
        ...
        download_app_files(get_general_file_path("patient", $file_info->patient_id), $file_data);
    }
    ...
    function delete_file() {
        ...
        if (!$info->patient_id || ($this->login_user->user_type == "patient" && $info->uploaded_by !== $this->login_user->id)) {
            redirect("forbidden");
        }

        $this->can_access_this_patient($info->patient_id);

        if ($this->General_files_model->delete($id)) {

            //delete the files
            delete_app_files(get_general_file_path("patient", $info->patient_id), array(make_array_of_file($info)));
    ...
    function contact_profile($contact_id = 0, $tab = "") {
        ...
        $this->can_access_this_patient($view_data['user_info']->patient_id);
        $view_data['patient_info'] = $this->Patients_model->get_one($view_data['user_info']->patient_id);
        $view_data['tab'] = $tab;
        if ($view_data['user_info']->user_type === "patient") {
            ...
            $this->template->rander("patients/contacts/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function save_personal_language($language) {
        if (!get_setting("disable_language_selector_for_patients") && ($language || $language === "0")) {
    ...
    function contacts($patient_id = 0) {
        $this->access_only_allowed_members();
        $this->can_access_this_patient($patient_id);
        ...
        if ($patient_id) {
            ...
            $view_data["patient_id"] = "";
            $view_data["view_type"] = "list_view";
        }
        $view_data["custom_field_headers"] = $this->Custom_fields_model->get_custom_field_headers_for_table("patient_contacts", $this->login_user->is_admin, $this->login_user->user_type);

        $view_data['can_edit_patients'] = $this->can_edit_patients();

        $this->load->view("patients/contacts/index", $view_data);
    }
    ...
    function add_new_contact_modal_form() {
        ...
        $view_data['model_info']->patient_id = $this->input->post('patient_id');
        $this->can_access_this_patient($view_data['model_info']->patient_id);

        $view_data["custom_fields"] = $this->Custom_fields_model->get_combined_details("patient_contacts", $view_data['model_info']->id, $this->login_user->is_admin, $this->login_user->user_type)->result();
        $this->load->view('patients/contacts/modal_form', $view_data);
    }
    ...
    function contact_general_info_tab($contact_id = 0) {
        if ($contact_id) {
            ...
            $this->can_access_this_patient($view_data['model_info']->patient_id);
            $view_data["custom_fields"] = $this->Custom_fields_model->get_combined_details("patient_contacts", $contact_id, $this->login_user->is_admin, $this->login_user->user_type)->result();

            $view_data['label_column'] = "col-md-2";
            $view_data['field_column'] = "col-md-10";
            $view_data['can_edit_patients'] = $this->can_edit_patients();
            $this->load->view('patients/contacts/contact_general_info_tab', $view_data);
        }
    }

    /* load contact's company info tab view */

    function company_info_tab($patient_id = 0) {
        if ($patient_id) {
            ...
            $this->load->view('patients/contacts/company_info_tab', $view_data);
        }
    }
    ...
    function contact_social_links_tab($contact_id = 0) {
        if ($contact_id) {
            ...
            $this->can_access_this_patient($contact_info->patient_id);

            $view_data['user_id'] = $contact_id;
            $view_data['user_type'] = "patient";
    ...
    function save_contact() {
        ...
        if (!$contact_id) {
            //inserting new contact. patient_id is required
        ...
        //by default, the first contact of a patient is the primary contact
        //check existing primary contact. if not found then set the first contact = primary contact
        $primary_contact = $this->Patients_model->get_primary_contact($patient_id);
        if (!$primary_contact) {
        ...
        if ($save_id) {
            ...
            save_custom_fields("patient_contacts", $save_id, $this->login_user->is_admin, $this->login_user->user_type);
    ...
    //save account settings of a patient contact (user)
    function save_account_settings($user_id) {
        ...
        if ($this->Users_model->save($account_data, $user_id)) {
            ...
            //resend new password to patient contact
    ...
    function save_profile_image($user_id = 0) {
        ...
        $this->can_access_this_patient($user_info->patient_id);
    ...
    function contacts_list_data($patient_id = 0) {

        $this->access_only_allowed_members_or_patient_contact($patient_id);
        $this->can_access_this_patient($patient_id);

        $custom_fields = $this->Custom_fields_model->get_available_fields_for_table("patient_contacts", $this->login_user->is_admin, $this->login_user->user_type);

        $options = array("user_type" => "patient", "patient_id" => $patient_id, "custom_fields" => $custom_fields, "show_own_patients_only_user_id" => $this->show_own_patients_only_user_id());
    ...
    private function _make_contact_row($data, $custom_fields, $hide_primary_contact_label = false) {
        ...
        $contact_link = anchor(get_uri("patients/contact_profile/" . $data->id), $full_name . $primary_contact) . $removal_request_pending;
        if ($this->login_user->user_type === "patient") {
            $contact_link = $full_name; //don't show clickable link to patient
        }

        $patient_info = $this->Patients_model->get_one($data->patient_id);

        $row_data = array(
            $user_avatar,
            $contact_link,
            anchor(get_uri("patients/view/" . $data->patient_id), $patient_info->company_name),
            $data->job_title,
            $data->email,
        ...
        $row_data[] = js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_contact'), "class" => "delete", "data-id" => "$data->id", "data-action-url" => get_uri("patients/delete_contact"), "data-action" => "delete"));
    ...
    function users() {
        if ($this->login_user->user_type === "patient") {
            $view_data['patient_id'] = $this->login_user->patient_id;
            $this->template->rander("patients/contacts/users", $view_data);
        }
    }
    ...
    function import_patients_modal_form() {
        $this->access_only_allowed_members();
        if (!$this->can_edit_patients()) {
            redirect("forbidden");
        }

        $this->load->view("patients/import_patients_modal_form");
    }

    private function _prepare_patient_data($data_row, $allowed_headers) {
        //prepare patient data
        $patient_data = array();
        $patient_contact_data = array("user_type" => "patient", "is_primary_contact" => 1);
        ...
        foreach ($data_row as $row_data_key => $row_data_value) { //row values
            ...
            } else if ($header_key_value == "patient_groups") { //we've to make patient groups data differently
                $patient_data["group_ids"] = $this->_get_patient_group_ids($row_data_value);
            } else if ($header_key_value == "contact_first_name") {
                $patient_contact_data["first_name"] = $row_data_value;
            } else if ($header_key_value == "contact_last_name") {
                $patient_contact_data["last_name"] = $row_data_value;
            } else if ($header_key_value == "contact_email") {
                $patient_contact_data["email"] = $row_data_value;
            } else {
                $patient_data[$header_key_value] = $row_data_value;
            }
        }

        return array(
            "patient_data" => $patient_data,
            "patient_contact_data" => $patient_contact_data,
    ...
    function save_patient_from_excel_file() {
        if (!$this->can_edit_patients()) {
            redirect("forbidden");
        }

        if (!$this->validate_import_patients_file_data(true)) {
            echo json_encode(array('success' => false, 'message' => lang('error_occurred')));
        }
        ...
        foreach ($excel_file as $key => $value) { //rows
            ...
            $patient_data_array = $this->_prepare_patient_data($value, $allowed_headers);
            $patient_data = get_array_value($patient_data_array, "patient_data");
            $patient_contact_data = get_array_value($patient_data_array, "patient_contact_data");
            $custom_field_values_array = get_array_value($patient_data_array, "custom_field_values_array");

            //couldn't prepare valid data
            if (!($patient_data && count($patient_data))) {
                continue;
            }

            //found information about patient, add some additional info
            $patient_data["created_date"] = $now;
            $patient_data["created_by"] = $this->login_user->id;
            $patient_contact_data["created_at"] = $now;

            //save patient data
            $patient_save_id = $this->Patients_model->save($patient_data);
            if (!$patient_save_id) {
                continue;
            }

            //save custom fields
            $this->_save_custom_fields_of_patient($patient_save_id, $custom_field_values_array);

            //add patient id to contact data
            $patient_contact_data["patient_id"] = $patient_save_id;
            $this->Users_model->save($patient_contact_data);
        }
    ...
    private function _save_custom_fields_of_patient($patient_id, $custom_field_values_array) {
        ...
        foreach ($custom_field_values_array as $key => $custom_field_value) {
            $field_value_data = array(
                "related_to_type" => "patients",
                "related_to_id" => $patient_id,
                "custom_field_id" => $key,
    ...
    private function _get_patient_group_ids($patient_groups_data) {
        $explode_patient_groups = explode(", ", $patient_groups_data);
        if (!($explode_patient_groups && count($explode_patient_groups))) {
            return false;
        }

        $groups_ids = "";

        foreach ($explode_patient_groups as $group) {
            $group_id = "";
            $existing_group = $this->Client_groups_model->get_one_where(array("title" => $group, "deleted" => 0));
            if ($existing_group->id) {
                //patient group exists, add the group id
                $group_id = $existing_group->id;
            } else {
                //patient group doesn't exists, create a new one and add group id
    ...
    function validate_import_patients_file() {
        $this->access_only_allowed_members();
    ...
    function validate_import_patients_file_data($check_on_submit = false) {
        $this->access_only_allowed_members();
                ...
                foreach ($headers as $row_data) {
                    ...
                    if (get_array_value($row_data, "has_error") && !$got_error_header) {
                        ...
                        if (get_array_value($row_data, "custom_field")) {
                            ...
                            $error_message = sprintf(lang("import_patient_error_header"), lang(get_array_value($row_data, "key_value")));
                        }
                    }
    ...
    private function _row_data_validation_and_get_error_message($key, $data, $has_contact_first_name) {
        ...
        if ($header_value == "company_name" && !$data) {
            return lang("import_patient_error_company_name_field_required");
        }
        ...
        if ($has_contact_first_name) {
            if ($header_value == "contact_last_name" && !$data) {
                return lang("import_patient_error_contact_name");
            }
            ...
            if ($header_value == "contact_email") {
                if ($data) {
                    ...
                    return lang("import_patient_error_contact_email");
                }
            }
        }
    }
    ...
    function download_sample_excel_file() {
        $this->access_only_allowed_members();
        download_app_files(get_setting("system_file_path"), serialize(array(array("file_name" => "import-patients-sample.xlsx"))));
    }

    function gdpr() {
        $view_data["user_info"] = $this->Users_model->get_one($this->login_user->id);
        $this->load->view("patients/contacts/gdpr", $view_data);
    }

    function export_my_data() {
        if (get_setting("enable_gdpr") && get_setting("allow_patients_to_export_their_data")) {
    ...
    private function _make_export_data($user_info) {
        ...
        $data .= "\n\n";
        $data .= strtoupper(lang("patient_info")) . "\n";

        //add company info
        $patient_info = $this->Patients_model->get_one($user_info->patient_id);
        $required_patient_info_array = array("company_name", "address", "city", "state", "zip", "country", "phone", "website", "vat_number");
        foreach ($required_patient_info_array as $field) {
            if ($patient_info->$field) {
                $data .= lang($field) . ": " . $patient_info->$field . "\n";
            }
        }
    ...
    function request_my_account_removal() {
        if (get_setting("enable_gdpr") && get_setting("patients_can_request_account_removal")) {

            $user_id = $this->login_user->id;
            $data = array("requested_account_removal" => 1);
            $this->Users_model->save($data, $user_id);

            $patient_id = $this->Users_model->get_one($user_id)->patient_id;
            log_notification("patient_contact_requested_account_removal", array("patient_id" => $patient_id), $user_id);

            $this->session->set_flashdata("success_message", lang("estimate_submission_message"));
            redirect("patients/contact_profile/$user_id/gdpr");
        }
    }

    /* load expenses tab  */

    function expenses($patient_id) {
        ...
        if ($patient_id) {
            ...
            $this->load->view("patients/expenses/index", $view_data);
        }
    }
