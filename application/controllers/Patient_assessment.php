...
class Patient_assessment extends MY_Controller {
    ...
    private function can_view_patient_assessment_contact_info() {
        if ($this->login_user->user_type == "staff") {
            if ($this->login_user->is_admin) {
                return true;
            } else if (get_array_value($this->login_user->permissions, "can_view_patient_assessment_contact_info") == "1") {
                return true;
            }
        }
    }

    private function can_view_patient_assessment_social_links() {
        if ($this->login_user->user_type == "staff") {
            if ($this->login_user->is_admin) {
                return true;
            } else if (get_array_value($this->login_user->permissions, "can_view_patient_assessment_social_links") == "1") {
                return true;
            }
        }
    }
    ...
    private function update_only_allowed_members($user_id) {
        if ($this->can_update_patient_assessment_info($user_id)) {
    ...
    private function can_update_patient_assessment_info($user_id) {
    ...
    public function index() {
        ...
        $view_data["show_contact_info"] = $this->can_view_patient_assessment_contact_info();

        $view_data["custom_field_headers"] = $this->Custom_fields_model->get_custom_field_headers_for_table("patient_assessment", $this->login_user->is_admin, $this->login_user->user_type);

        $this->template->rander("patient_assessment/index", $view_data);
    }
    ...
    function modal_form() {
        ...
        $view_data["custom_fields"] = $this->Custom_fields_model->get_combined_details("patient_assessment", 0, $this->login_user->is_admin, $this->login_user->user_type)->result();

        $this->load->view('patient_assessment/modal_form', $view_data);
    }
    ...
    function add_team_member() {
        ...
        if ($user_id) {
            ...
            save_custom_fields("patient_assessment", $user_id, $this->login_user->is_admin, $this->login_user->user_type);
    ...
    function invitation_modal() {
        $this->access_only_admin();
        $this->load->view('patient_assessment/invitation_modal');
    }
    ...
    function list_data() {
        ...
        $custom_fields = $this->Custom_fields_model->get_available_fields_for_table("patient_assessment", $this->login_user->is_admin, $this->login_user->user_type);
        $options = array(
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $show_cotact_info = $this->can_view_patient_assessment_contact_info();
        ...
        if ($this->login_user->is_admin && $this->login_user->id != $data->id) {
            $delete_link = js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_team_member'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("patient_assessment/delete"), "data-action" => "delete-confirmation"));
        }
    ...
    function view($id = 0, $tab = "") {
        if ($id * 1) {
            ...
            if (!$this->can_view_patient_assessment_list() && $this->login_user->id != $id) {
                redirect("forbidden");
            }
            ...
            if ($user_info) {
                ...
                $can_update_patient_assessment_info = $this->can_update_patient_assessment_info($id);

                $view_data['show_general_info'] = $can_update_patient_assessment_info;
                ...
                //check contact info view permissions
                $show_cotact_info = $this->can_view_patient_assessment_contact_info();
                $show_social_links = $this->can_view_patient_assessment_social_links();
                ...
                $this->template->rander("patient_assessment/view", $view_data);
            } else {
                show_404();
            }
        } else {

            if (!$this->can_view_patient_assessment_list()) {
                redirect("forbidden");
            }

            //we don't have any specific id to view. show the list of team_member
            $view_data['patient_assessment'] = $this->Users_model->get_details(array("user_type" => "staff", "status" => "active"))->result();
            $this->template->rander("patient_assessment/profile_card", $view_data);
        }
    }
    ...
    function job_info($user_id) {
        ...
        $this->load->view("patient_assessment/job_info", $view_data);
    }
    ...
    function general_info($user_id) {
        ...
        $view_data["custom_fields"] = $this->Custom_fields_model->get_combined_details("patient_assessment", $user_id, $this->login_user->is_admin, $this->login_user->user_type)->result();

        $this->load->view("patient_assessment/general_info", $view_data);
    }
    ...
    function my_preferences() {
        ...
        if (!get_setting("disable_language_selector_for_patient_assessment")) {
        ...
        $this->load->view("patient_assessment/my_preferences", $view_data);
    }
    ...
    function save_personal_language($language) {
        if (!get_setting("disable_language_selector_for_patient_assessment") && ($language || $language === "0")) {
    ...
    function projects_info($user_id) {
        if ($user_id) {
            ...
            $this->load->view("patient_assessment/projects_info", $view_data);
        }
    }
    ...
    function attendance_info($user_id) {
        if ($user_id) {
            $view_data['user_id'] = $user_id;
            $this->load->view("patient_assessment/attendance_info", $view_data);
        }
    }

    //show weekly attendance list of a team member
    function weekly_attendance() {
        $this->load->view("patient_assessment/weekly_attendance");
    }

    //show weekly attendance list of a team member
    function custom_range_attendance() {
        $this->load->view("patient_assessment/custom_range_attendance");
    }

    //show attendance summary of a team member
    function attendance_summary($user_id) {
        $view_data["user_id"] = $user_id;
        $this->load->view("patient_assessment/attendance_summary", $view_data);
    }
    ...
    function leave_info($applicant_id) {
        if ($applicant_id) {
            ...
            $this->load->view("patient_assessment/leave_info", $view_data);
        }
    }

    //show yearly leave list of a team member
    function yearly_leaves() {
        $this->load->view("patient_assessment/yearly_leaves");
    }

    //show yearly leave list of a team member
    function expense_info($user_id) {
        $view_data["user_id"] = $user_id;
        $this->load->view("patient_assessment/expenses", $view_data);
    }
    ...
    function files($user_id) {
        ...
        $view_data['user_id'] = $user_id;
        $this->load->view("patient_assessment/files/index", $view_data);
    }
    ...
    function file_modal_form() {
        ...
        $view_data['user_id'] = $user_id;
        $this->load->view('patient_assessment/files/modal_form', $view_data);
    }
    ...
    function save_file() {
        ...
        $target_path = getcwd() . "/" . get_general_file_path("patient_assessment", $user_id);
    ...
    private function _make_file_row($data) {
        ...
        $description = "<div class='pull-left'>" .
                js_anchor(remove_file_prefix($data->file_name), array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "data-url" => get_uri("patient_assessment/view_file/" . $data->id)));
        ...
        $options = anchor(get_uri("patient_assessment/download_file/" . $data->id), "<i class='fa fa fa-cloud-download'></i>", array("title" => lang("download")));

        if ($this->login_user->is_admin || $data->uploaded_by == $this->login_user->id) {
            $options .= js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_file'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("patient_assessment/delete_file"), "data-action" => "delete-confirmation"));
        }
    ...
    function view_file($file_id = 0) {
        ...
        if ($file_info) {
            ...
            $view_data["file_url"] = get_source_url_of_file(make_array_of_file($file_info), get_general_file_path("patient_assessment", $file_info->user_id));
            ...
            $this->load->view("patient_assessment/files/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function download_file($id) {
        ...
        download_app_files(get_general_file_path("patient_assessment", $file_info->user_id), $file_data);
    }
    ...
    function delete_file() {
        ...
        if ($info->user_id && ($this->login_user->is_admin || $this->login_user->id === $info->uploaded_by)) {
            if ($this->General_files_model->delete($id)) {
                ...
                //delete the files
                delete_app_files(get_general_file_path("patient_assessment", $info->user_id), array(make_array_of_file($info)));
    ...
    function keyboard_shortcut_modal_form() {
        $this->load->view('patient_assessment/keyboard_shortcut_modal_form');
    }
